...
const li = 6.35; // light increment
const di = 5.35; // dark increment
export const paletteHueShift = (hex) => {
    ...
    let i = 1;
    //  light
    while (lightValues.length < (li - 2) || i <= 255) {
        ...
        if(newRl > (lightValues.length + 1) * lightIncrement + rl && lightValues.length < 5 ){
            lightValues.push(hx);
        }
        i++
    }
...
export const paletteFromLightness = (hex, ) => {
    ...
    while (darkValues.length < dl - 2 && i > 0) {
        const H = hsl[0]
        const S = hsl[1]
        const L = i
        const hx = HSLToHex([H,S,L])
